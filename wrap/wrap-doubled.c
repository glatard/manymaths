...
float logf (float x) { 
  float y = (float) log((double) x);
  if(DEBUG)
    printf("[WRAP-DOUBLED] %s %13a %13a\n", "logf", x, y);
  return y; 
  }
...
float atan2f (float x, float y) { 
  float z = (float) atan2((double) x, (double) y);
  if(DEBUG)
    printf("[WRAP-DOUBLED] %s %13a %13a %13a\n", "atan2f", x, y, z);
  return z; 
  }
